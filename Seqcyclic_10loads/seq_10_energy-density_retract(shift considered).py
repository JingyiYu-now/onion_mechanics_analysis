...
# total_chart = pd.DataFrame(columns=loadx)
...
# stack bar chart
#     bx.bar(xlist[g], C_s, yerr = Ce_s, color = 'C2', alpha = 0.6, capsize = 5, hatch = h[g])
#     bx.bar(xlist[g], C_h,  yerr = Ce_h, bottom = C_s, color = 'C3', alpha = 0.6, capsize = 5, hatch = h[g])
#     bottom2 = [x+y for x, y in zip(C_h, C_s)]
#     bx.bar(xlist[g], C_p, yerr = Ce_p, bottom= bottom2, color = 'C9', alpha = 0.6, capsize = 5, hatch = h[g])
# bx.legend(['Stored energy', 'Dissipated energy(Elastic hysteresis)', 'Dissipated energy(Plasticity)'], loc =2)
# bx.grid(True, alpha = 0.4, linestyle = '--')
bx.set_yticks(np.arange(0, 1.4, 0.2))
bx.set_xticks(xlabel)
bx.set_xticklabels(xlabel)
bx.set(ylim = [-0.2, 1.2], xlabel='Stress (MPa)', ylabel='Strain energy density (MPa)')
plt.gcf().subplots_adjust(bottom=0.15)
plt.gcf().set_size_inches(6.4, 5.5)
# g =+ 1
# plt.axhline(y = 0, color = 'black')
plt.savefig(''f'{output}/Energy_density_evolution.pdf', transparent = True)
plt.show()
