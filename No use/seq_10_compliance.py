...
t_comp = pd.DataFrame(columns=loadx)
e_comp = pd.DataFrame(columns=loadx)
p_comp = pd.DataFrame(columns=loadx)
r_comp = pd.DataFrame(columns=loadx)
h_comp = pd.DataFrame(columns=loadx)
...
plt.errorbar(x, C_e, yerr = er_e, color='C1', alpha = 0.6, marker = mark, markersize = ms)
plt.errorbar(x, C_p, yerr = er_p, color='C9', alpha = 0.6, marker = mark, markersize = ms, linewidth = lw, capsize=5, markeredgewidth= lw)
# plt.errorbar(x, C_r, yerr = er_e, color='C2', alpha = 0.6, marker = mark, markersize = ms)
# plt.errorbar(x, C_h, yerr = er_h, color='C3', alpha = 0.6, marker = mark, markersize = ms, linewidth = lw, capsize=5, markeredgewidth= lw)

# plot without error bar, without total value
# ms = 4
# plt.plot(x,C_t, color = 'C7', alpha = 0.6, marker = mark, markersize = ms)
# plt.plot(x,C_p, color = 'C9', alpha = 0.6, marker = mark, markersize =ms)
# plt.plot(x,C_h, color = 'C3', alpha = 0.6, marker = mark, markersize = ms)
...
ax.set(ylim = [-1, 5], xlabel='Stress (MPa)', ylabel='Compliance (∆L%/MPa)')
t_line = mlines.Line2D([], [], color = 'C7', label = 'Total compliance')
green_line = mlines.Line2D([], [], color = 'C9', label = 'Plastic compliance')
# orange_line = mlines.Line2D([], [], color = 'C3', label = 'Hysteresis compliance')
...
ax.legend(handles = [t_line, green_line], loc = 2, fontsize=10, frameon = False)
...
# plt.savefig(''f'{output}/Compliance_evolution.pdf', transparent = True)
plt.show()
