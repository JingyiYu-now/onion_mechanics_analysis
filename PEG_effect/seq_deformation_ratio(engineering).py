...
g = 0
mark = '8'
t_defo_ratio = pd.DataFrame(columns=loadx)
e_defo_ratio = pd.DataFrame(columns=loadx)
pr_defo_ratio = pd.DataFrame(columns=loadx)
p_defo_ratio = pd.DataFrame(columns=loadx)
real_e_defo_ratio = pd.DataFrame(columns=loadx)
t_inc_defo_ratio = pd.DataFrame(columns=loadx)
e_inc_defo_ratio = pd.DataFrame(columns=loadx)
p_inc_defo_ratio = pd.DataFrame(columns=loadx)
...
for folder in group:
    ...
    # for sequential Instron - elastic deformation
        ...
        for i in range(len(curve) - 1):
            ...
            ela_defo_ratio.append((curve[i].position[len(curve[i]) - 1] - ten_p(curve[i + 1])) / 5000 * 100 / total_defo[i])
        e_defo_ratio.loc[len(e_defo_ratio)] = ela_defo_ratio
    ...
    # for sequential Instron - plastic deformation
        ...
        for i in range(len(curve) - 1):
            ...
            pla_defo_ratio.append((ten_p(curve[i + 1]) - ori_p) / 5000 * 100 / total_defo[i])
        # pla_forplot = [ -x for x in pla_defo]
        p_defo_ratio.loc[len(p_defo_ratio)] = pla_defo_ratio
    ...
    # for sequential Instron - recoverable elastic deformation
        ...
        for i in range(len(curve) - 1):
            ...
            pla_rc_defo_ratio.append(((ten_p(retract[i]) - ten_p(curve[i + 1])) / 5000 * 100 )/total_defo[i])

        pr_defo_ratio.loc[len(e_defo_ratio)] = pla_rc_defo_ratio
    ...
    # real elastic deformation
        ...
        for i in range(len(curve) - 1):
            ...
            real_ela_defo_ratio.append((ela_defo[i] - pla_rc_defo[i])/total_defo[i])
        real_e_defo_ratio.loc[len(e_defo_ratio)] = real_ela_defo_ratio
    ###### results plotting
    ##### set x
    # n = 1
    # t = 3
...
for i in range(len(loadx)):
    # C_t.append(np.mean(t_defo_ratio.iloc[:, i]))
    # C_e.append(np.mean(e_defo.iloc[:, i]))
    C_re.append(np.mean(real_e_defo_ratio.iloc[:, i]))
    C_p.append(np.mean(p_defo_ratio.iloc[:, i]))
    C_er.append(np.mean(pr_defo_ratio.iloc[:, i]))
    # er_t.append(np.std(t_defo.iloc[:, i]))
    er_re.append(np.std(real_e_defo_ratio.iloc[:, i]))
    er_p.append(np.std(p_defo_ratio.iloc[:, i]))
    er_er.append(np.std(pr_defo_ratio.iloc[:, i]))
...
ax.set(ylim = [0,1], xlabel='Stress (MPa)', ylabel='Deformation ratio')
...
ax.legend(handles = [C7_line, orange_line, C12_line], loc = 2, fontsize=10, frameon = False)
...
# plt.savefig(''f'{output}/Strain_ratio_evolution.pdf', transparent = True)
plt.show()