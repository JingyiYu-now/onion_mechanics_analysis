...
ori_pull = whole_df2[ind(whole_df, 'STRAIN [MICRONS]') + 1:].astype(float).reset_index(drop=True)
...
bx = plt.subplot(212)
bx.set(xlim = [l,r], xlabel = 'Strain(%)', ylabel = 'Derivative(MPa)')
...
# for line, text in zip(leg.get_lines(), leg.get_texts()):
#     text.set_color(line.get_color())
plt.savefig(''f'{output}/True_engineering_curve.pdf', transparent = True)
plt.show()
